exports.process = function (xmlObj) {
    ...
    result['objectives']['blue wait'] = false;
...
xmlObj.project.stage[0].sprites[0].sprite.forEach(function(u){
          ...
          u.scripts[0].script.forEach(function(v){
            if (v.block[0].$.s == "receiveMessage"){
               result['objectives']['rcv blue msg']  = true;
                ...
                if (v.block[1] != null && v.block[1].$.s == "doWait"){
                  result['objectives']['blue wait'] = true;
                  if (v.block[2] != null && (v.block[2].$.s == "doGlideSteps" || (v.block[2].$.s == "doGlideDirection" && v.block[2].l[1] == "up"))){ 
                    result['objectives']['B glide up'] = true;
                    var numSteps = parseInt(v.block[2].l[0]);
                    if (numSteps >= 130 && numSteps <= 180)
                      result['objectives']['B numSteps'] = true;
                    if (v.block[3]!= null && (v.block[3].$.s == "doGlideDirection" && v.block[3].l[1] == "left")) 
                    {
                      result['objectives']['turn left'] = true;
                      var stepsLeft = parseInt(v.block[3].l[0]);
                      if (stepsLeft >= 175 && stepsLeft <= 250)
                        result['objectives']['target blue'] = true;
                      else leftNumSteps = false;
                    }
                  else{
                    if (v.block[3]!= null && ((v.block[3].$.s == "doFaceTowards" && v.block[3].l[0] == "Target Blue") || (v.block[3].$.s == "setHeading" && v.block[3].l[0] == "left"))){
                      result['objectives']['turn left'] = true;
                    if (v.block[4]!= null && (v.block[4].$.s == "doSpeedGlidetoObject" && v.block[4].l[1] == "Target Blue") || (v.block[4].$.s == "doGlidetoObject" && v.block[4].l[0] == "Target Blue"))
                      result['objectives']['target blue'] = true;}}}
    ...
    else if (!result['objectives']['blue wait'])
      result.html = "Uh Oh, there's a traffic jam! The blue car hit the yellow car... Try again but this time put a 'wait' block before the blue car's motion block! That will help avoid a crash! Keep trying, you're getting there!";
        ...
        result.html = " You're so close! When the yellow cars glide up, it looks like it is not taking the right number of steps! Try gliding to the center of the intersection and then turning! This should take between 150 and 200 steps. Keep trying!";
    else if (!result['objectives']['B numSteps'])
      result.html = "You're so close! The blue car is not moving far enough up through the intersection before turning to the left. It has to glide between 130 and 180 steps to be in the center of the intersection before turning. Keep trying!";
        ...
        result.html = "You are so close! It looks like the blue car is not moving enough steps to reach the blue target. Make sure it is taking between 175 and 250 steps to the left! Keep trying!";

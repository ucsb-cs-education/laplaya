exports.process = function (xmlObj) {
    ...
    // new checks to reset itams
    result['objectives']['place at'] = false;
    result['objectives']['start'] = false;
    result.html = "";
try{
xmlObj.project.stage[0].sprites[0].sprite.forEach(function(u){
                ...
                if (v.block[1].script[0] != null && v.block[1].script[0].block[0].$.s == "doIf"){
                  ...
                  if (v.block[1].script[0].block[0].block[0].$.s == "reportTouchingObject"){
                    ...
                    if (v.block[1].script[0].block[0].script[0].block[0].$.s == "doSayFor"){
                      ...
                      if (v.block[1].script[0].block[0].script[0].block[1] != null && v.block[1].script[0].block[0].script[0].block[1].$.s == "doGotoObject"){
                        result['objectives']['place at'] = true;
                        if (v.block[1].script[0].block[0].script[0].block[1].l[0] == "Start")
                          result['objectives']['start'] = true;
                      }
                    }
                    else if (v.block[1].script[0].block[0].script[0].block[0].$.s == "doGotoObject"){
                       result['objectives']['place at'] = true;
                       if (v.block[1].script[0].block[0].script[0].block[0].l[0] == "Start")
                          result['objectives']['start'] = true;
                        if (v.block[1].script[0].block[0].script[0].block[1] != null && v.block[1].script[0].block[0].script[0].block[1].$.s == "doSayFor")
                          result['objectives']['say'] = true;
                    }
                  }
                }
    ...
    else if (result['objectives']['place at'] == false)
      result.html = "Be sure to use the 'place at' motion block inside the if statement to reset the explorer to the start if the explorer is touching the wall! Keep trying!";
    else if (result['objectives']['start'] == false)
      result.html = "Be sure to place the explorer back to the start by using the 'Place at' block and finding 'Start' on the dropdown list. You're almost done!";
